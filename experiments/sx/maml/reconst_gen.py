...
from trainers.trainer_sine_anil import eval_on_task
...
if __name__ == '__main__':
    ...
    logging.basicConfig(
        filename=os.path.basename(__file__)[:-3] + '_' + enc + '_' + params['dataset'] + '_' + str(
            wanted_sa) + '.log',
        level=logging.INFO,
    ...
    params['model'] = enc + '_' + str(params['dataset'])
    ...
    if params['encoder']:
        ...
        tasksets = BPDataset(dataset=params['dataset'], num_tasks=num_tasks, meta_train=True,
                             ...
                             noise_std=params['noise_std'], random_order=['random_order'])
    ...
    logging.info('** params ***')
    torch.save(encoder, 'encoder_{}.pth'.format(params['underlying_seed']))
