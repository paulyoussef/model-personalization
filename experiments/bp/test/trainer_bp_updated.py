...
def train_maml(
    ...
    shots = params['shots']
    params['maml_bias'] = str(model.head[3].bias).split('\n')[1]
...
def train_baseline(scaler,
    ...
    params['baseline_bias'] = str(model.head[3].bias).split('\n')[1] # str(model.head[6].bias).split('\n')[1]
...
def eval_on_task(model, scaler, support, query, params, results_dict):
    ...
    xquery, yquery = query
    assert (len(xquery) == len(yquery))
    assert (len(xquery) == shots)


    # Evaluation is done
    # We can unscale
    # 1 refers to the window size here
    assert (xquery.shape[1] == 4) # steps_ahead + windowsize
