...
from trainers.trainer_bp import eval_on_task # TODO check this
...
if __name__ == '__main__':
    ...
    tasksets = BPDataset(dataset=params['dataset'], num_tasks=num_tasks, meta_train=True,
                         ...
                         static_config=params['static_config'], include_static=params['with_static'], random_order=['random_order'])
    ...
    if params['encoder']:
        ...
        for i in range(n):
            ...
            Y.append(get_pers_repr_generic(i, dir, params['device'], only='head'))
    ...
    for h in range(11):
        ...
        params['ft_points'] = h
        params['ft_steps'] = 0 if params['ft_points'] == 0 else 10
        params['model'] = enc + '_' + str(params['dataset']) + '_' + str(params['ft_points'])
        ...
        for i, t in enumerate(test_tasksets):
            ...
            if params['encoder']:
                with torch.no_grad():
                    ...
                    m_i = overwrite_model_weights_generic(m_i, repr, params['dtype'], only='head')
            # Inputs have shape (20, 128)
            inputs_np, targets_np = t.get_scaled_inputs_targets()

            inputs = torch.tensor(np.array(inputs_np), dtype=params['dtype']).to(params['device'])
            targets = torch.tensor(np.array(targets_np), dtype=params['dtype']).to(params['device'])
